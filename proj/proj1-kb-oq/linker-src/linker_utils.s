...
relocate_inst:
	...
	move $a0, $s3
	jal symbol_for_addr		#gets name from relocation table
	beq $v0, $0, error
	move $a1, $v0
	move $a0, $s2
	jal addr_for_symbol		#get global address from symbol table
	li $t0, -1
	beq $t0, $v0, error
	srl $v0, $v0, 2
	andi $v0, $v0, 0x03ffffff
	andi $t1, $s0, 0xfc000000
	or $v0, $t1, $v0		#adds address into instruction
