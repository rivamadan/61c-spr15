...
add_to_list:	
	...
	jal new_node		#get a new node in $v0
	move $s3, $v0		#move node to $s3
	sw $s1, 0($s3)		#store the address
	sw $s0, 8($s3)		#store the current ptr to list as next
	move $a0, $s2		#move name to argument to copy the string
	jal copy_of_str
	sw $v0, 4($s3)		#store the pointer to a string that copy_of_str returns
