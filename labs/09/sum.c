...
static int sum_vectorized(int n, int *a)
{
	...
	for (int i = 0; i < n/4 * 4; i += 4)
	{
		__m128i b = _mm_loadu_si128((__m128i*) (a + i));
		vec_sum = _mm_add_epi32(vec_sum, b);
	}

	int p[4] = {0, 0, 0, 0};
	_mm_storeu_si128((__m128i*) p, vec_sum);
...
static int sum_vectorized_unrolled(int n, int *a)
{
	...
	for (int i = 0; i < n / 16 * 16; i += 16)
	{
		...
		b = _mm_loadu_si128((__m128i*) (a + i+4));
		vec_sum = _mm_add_epi32(vec_sum, b);

		b = _mm_loadu_si128((__m128i*) (a + i+8));
		vec_sum = _mm_add_epi32(vec_sum, b);

		b = _mm_loadu_si128((__m128i*) (a + i+12));
		vec_sum = _mm_add_epi32(vec_sum, b);
	}
