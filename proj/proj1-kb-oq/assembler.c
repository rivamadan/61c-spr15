...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    int error_check;
    int check;
    int error_num_args = 0; 
    int error = 0;

    while(fgets(line, sizeof(line), input)) {
        skip_comment(line);
        line_number++;
        char * str = strtok(line, IGNORE_CHARS);
        ...
        error_check = add_if_label(line_number, str, byte_offset, symtbl);
        
        char* next = strtok(NULL, IGNORE_CHARS);

        //if only label on the line
        if (next == NULL) {
            continue; 
        }
        char* args[MAX_ARGS];
        ...
        while (str != NULL) {
            ...
            if  (num_args > MAX_ARGS) {
                raise_extra_arg_error(line_number, str); 
                error_num_args = 1; 
                error = 1;
                break;
            }
        ...
        if (!error_num_args) {
                check = write_pass_one(output, instruction, args, num_args);
            if (check == 0) {
                error = -1;
                raise_inst_error(line_number, instruction, args, num_args); 
            }
            else  {
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    // go to the next line.
        ...
        addr = byte_offset; 
        if (translate_inst(output, instruction, args, num_args, addr, symtbl, reltbl) == -1) {
            err = -1; 
            raise_inst_error(line_number, instruction, args, num_args); 
        }
